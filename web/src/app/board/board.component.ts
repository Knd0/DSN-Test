...
export class BoardComponent implements OnInit {
  ...
  createTask() {
    ...
    fetch('https://www.dsn-test-production.up.railway.app/tasks', {
      method: 'POST',
  ...
  drop(event: CdkDragDrop<Task[]>, targetCol: Column) {
    ...
    fetch(`https://www.dsn-test-production.up.railway.app/tasks/${task.id}/move`, {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
  ...
  const board = { ...this.socket.board() };
  (['todo', 'doing', 'done'] as Column[]).forEach(col => {
    board[col] = board[col].map(t => (t.id === taskId ? updatedTask : t));
  });
  this.socket['_board'].set(board);

  // Llamada al backend para persistir cambios
  fetch(`https://www.dsn-test-production.up.railway.app//tasks/${taskId}`, {
    method: 'PATCH',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({
  ...
  fetch(`https://www.dsn-test-production.up.railway.app//tasks/${taskId}`, {
    ...
    this.socket['_board'].set(board); // actualizamos el signal
